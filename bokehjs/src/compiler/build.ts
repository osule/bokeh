...
export async function init(base_dir: Path, _bokehjs_dir: Path, base_setup: InitOptions): Promise<boolean> {
  ...
  const setup: Required<InitOptions> = {
    ...
    development: !!base_setup.development,
  ...
  const bokehjs = (() => {
    if (setup.development) {
      const bokehjs_dir = dirname(dirname(__dirname))
      if (file_exists(join(bokehjs_dir, "package.json"))) {
        const rel_path = relative(base_dir, bokehjs_dir)
        return `file://${rel_path}`
      } else
        print("Can't find bokehjs' directory.")
    }

    const version = setup.bokeh_version.split("-")[0]
    return `^${version}`
  })()
